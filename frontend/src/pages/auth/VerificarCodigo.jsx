...
function VerificarCodigo() {
  ...
  useEffect(() => {
    ...
    if (location.state?.mensaje) setMensaje(location.state.mensaje);
  }, [location.state]);
  ...
  const handleVerificar = async (e) => {
    ...
    if (!email.trim()) {
      setError("Escribe tu correo antes de verificar");
      setCargando(false);
      return;
    }
    ...
    try {
      await api.post("/auth/verificar-codigo", { email: email.trim(), codigo });
      setVerificado(true);
  ...
  const reenviarCodigo = async () => {
    ...
    if (!email.trim()) {
      setError("Escribe tu correo antes de reenviar el código");
      setCargando(false);
      return;
    }
    ...
    return (
      <div className="vc-wrap">
        ...
        <div className="vc-card vc-card--success">
          ...
          <div className="vc-loading-bar">
            <div className="vc-loading-fill" />
          </div>
        </div>
      </div>
    );
    ...
    <div className="vc-wrap">
      ...
      <div className="vc-card">
        ...
        <p className="vc-subtitle">
          Escribe tu correo y luego ingresa el código de 6 dígitos.
        </p>
        ...
        <form className="vc-form" onSubmit={handleVerificar}>
          ...
          <div className="vc-email-field">
            <label className="vc-label">Correo electrónico</label>
            <input
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="vc-email-input"
          ...
          <div className="vc-dots">
            {digits.map((d, i) => (
              <div key={i} className={`vc-dot ${d ? "vc-dot-filled" : ""}`} />
            ))}
          </div>
